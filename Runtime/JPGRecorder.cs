...
namespace NatML.Recorders {
    ...
    public sealed class JPGRecorder : IMediaRecorder {
        ...
        public unsafe void CommitFrame (void* pixelBuffer, long timestamp = default) {
            ...
            var task = Task.Run(() => File.WriteAllBytes(Path.Combine(recordingPath, $"{frameIndex}.jpg"), buffer.ToArray()));
            writeTasks.Add(task);
        }
        ...
        unsafe void IMediaWriter.CommitSamples (float[] sampleBuffer, long timestamp) {
            fixed (float* baseAddress = sampleBuffer)
                (this as IMediaRecorder).CommitSamples(baseAddress, sampleBuffer.Length, timestamp);
        }

        unsafe void IMediaWriter.CommitSamples (NativeArray<float> sampleBuffer, long timestamp) {
            (this as IMediaRecorder).CommitSamples((float*)sampleBuffer.GetUnsafeReadOnlyPtr(), sampleBuffer.Length, timestamp);
        }

        unsafe void IMediaWriter.CommitSamples (float* sampleBuffer, int sampleCount, long timestamp) {
